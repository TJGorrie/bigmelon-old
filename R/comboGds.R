...
# Forcefully coerces secondary gds node to mirror primarys dimensions forcing NA (if necessary).
...
combo.gds <- function(file, primary, secondary){
  ...
  # forces row length of largest dimension (unless they are the same length, then primary is taken
  forcerow <- rownames(primary)

#  rn <- list()
#  rn[[1]] <- rownames(primary)
#  rn[[2]] <- rownames(secondary)
#  if(length(rn[[1]])==length(rn[[2]])){ 
#    forcerow <- rn[[1]]
#  } else { # Initiate clown script
#    forcerow <- rn[[which( 
#                    max( length(rn[[1]]), length(rn[[2]]) ) == c( length(rn[[1]]), length(rn[[2]]) ) 
#                    )]]
#  }
  ...
  for(x in c("betas", "pvals", "methylated", "unmethylated", "NBeads")){
    if( x%in% sharenod){
      ...
      if(getTrait(index.gdsn(primary, x)) == getTrait(index.gdsn(secondary, x))){
      ...
      for(m in 1:primdim[2]){ # This will coerce NA's if probes are missing.
  ...
  # If fData is missing from one node, do not append fData(?) - Assumption is
  # that if fData is missing, this operation cannot be completed anyway.
  } else {
  ...
  # Match Colnames e.t.c?
#  pDPrim <- pData(primary)
#  pDSeco <- pData(secondary)
#  pD <- rbind(pDPrim, pDSeco)
  ...
  for(x in c("QCmethylated", "QCunmethylated")){
    if(x %in% sharenod){
      ...
      n.t <- add.gdsn(bmln, x, valdim = c(length(QCrownames(primary)), 0), val = NULL, replace = TRUE, storage="int32")
  ...
  history.command <- paste0("Combo of: ", primary[[1]], " and ", secondary[[1]], ".")
