###
 # Normalizes both meth and unmeth values together.
 # Mirrors wateRmelon::db1
###
 # Used by:
 # 'nanet.gds'; 'danet.gds'; 'nanes.gds'; 'danes.gds' | location: dasenGds.R
 # 
###
 # Arguments:
 # gds      : gds object 
 # mns      : gdsn.class object corresponding to methylated intensities, usually methylated(gds)
 # uns      : gdsn.class object corresponding to unmethylated intensities, usually unmethylated(gds)
 #
###
 # Note:
 # Requires root gds objects to be opened in readonly=F. If calling the above functions
 # this is already checked. 
###

db.gdsn <- function(gds, mns, uns){ # {{{
  # This function is called to by nanet(), danet(), nanes() and danes() e.t.c
  # where-by gds can be a temporary node (and not the original where mns and uns come from))
  ...
  for(x in seq(1,(2*dim[2])-1, 2)){ 
    val <- readex.gdsn(index.gdsn(gds, path = "db"), sel = list(NULL, c(x, x + 1)))
