es2gds <- function(m, file, qc = TRUE){
  # Not memory efficient as we are working from pre-loaded Expression Set derivatives.
  history.submitted <- as.character(Sys.time())
  if (!inherits(m, c('MethylSet', 'MethyLumiSet', 'RGChannelSet'))){
    stop(paste(deparse(substitute(m)), "is not a MethyLumiSet or MethylSet or RGChannelSet object"))
  }
  ...
  if(inherits(m, 'MethyLumiSet')){
    ...
    add.gdsn(bmln, "fData", 
    val = data.frame(lapply(fData(m), as.character), stringsAsFactors = FALSE), check = FALSE)
    message('pData... ' )
    add.gdsn(bmln, "pData", 
    val = data.frame(lapply(pData(m), as.character), stringsAsFactors = FALSE))
    message('qcdata... ' )
    if (qc) {
      if(!is.null(m@QC)){
        add.gdsn(bmln, "QCmethylated", val = a <- methylated(QCdata(m)))
        add.gdsn(bmln, "QCunmethylated", val = unmethylated(QCdata(m)))
    ...
    val = data.frame(lapply(m@history, as.character), stringsAsFactors = FALSE))
  ...
  if(inherits(m, 'MethylSet')){
    ...
    # fData
    message('fData... ')
    fd <- data.frame(lapply(getAnnotation(m), as.character), stringsAsFactors = FALSE)
    add.gdsn(bmln, "fData", val = fd, check = FALSE)
    message('pData... ')
    # Assuming data-less pData(m)
    pd <- data.frame(lapply(cbind(rownames(pData(m)), pData(m)), as.character), stringsAsFactors = FALSE)
    ...
    message('finished creating gdsfile' )
    add.gdsn(bmln, "history", 
    val = data.frame(lapply(data.frame(submitted = "", finished = "", command = ""), as.character), stringsAsFactors = FALSE))
  ...
  if(inherits(m, 'RGChannelSet')){
    ...
    if(qc){
      ...
      add.gdsn(bmln, "QCmethylated", val = a <- getGreen(m)[ctrls$Address, ])
      add.gdsn(bmln, "QCunmethylated", val = getRed(m)[ctrls$Address, ])
      add.gdsn(bmln, "QCrownames", val = rownames(a))
    } else { 
      message('     ...skipped')
    }
    ...
    add.gdsn(bmln, "history", 
             val = data.frame(
                     lapply(
                       data.frame(submitted = "", finished = "", command = ""),
                       as.character
                     ),
                     stringsAsFactors = FALSE
                   )
  ...
  # END RGChannelSet(Extended) Method
    ...
    for(i in c("targetID", "name", "Probe_ID")){
      rown[[i]] <- grep(i, ls.gdsn(index.gdsn(bmln, "fData")), ignore.case = TRUE)
    }
    rowna <- ls.gdsn(index.gdsn(bmln, "fData"))[unlist(rown)[1]]
    coln <- list()
    # Change search terms - in any event!
    for(i in c("sampleID", "barcode", "name")){ 
      coln[[i]] <- grep(i, ls.gdsn(index.gdsn(bmln, "pData")), ignore.case = TRUE)
    }
    ...
    message('Directing \'colnames\' to ', paste0("pData/", colna),
            ' by default, change with redirect.gds if incorrect.')
    add.gdsn(bmln, "paths", val = c(paste0("fData/", rowna), paste0("pData/", colna)))
