###
 # Huge collection of accessory functions and methods for various wateRmelon functions
 #

redirect.gds<- function(gds, rownames, colnames){
  # Wrapper to change the gdsn node "paths" where row and colnames are stored.
  a <- try(index.gdsn(gds, rownames), silent = TRUE)
  if(inherits(a, "try-error")) stop(paste(rownames, "does not exist in gds object!"))
  b <- try(index.gdsn(gds, colnames), silent = TRUE)
  if(inherits(b, "try-error")) stop(paste(colnames, "does not exist in gds object!"))  
...
backup.gdsn <- function(gds = NULL, node){
  ...
  # called "backup". Really for personal use - not recommended for standard workflow.
...
'[.gdsn.class' <- function(x, i, j, name = TRUE, drop = TRUE){ # {{{
  ...
  #    nrow <- rep(FALSE, objdesp.gdsn(x)$dim[1])
    nrow <- i
...
setMethod(
  ...
  definition = function(x, do.NULL = TRUE, prefix = NULL){ 
    read.gdsn(index.gdsn(getfolder.gdsn(x), read.gdsn(index.gdsn(getfolder.gdsn(x), "paths"))[2]))
  }
)
...
setMethod(
  ...
  signature(x = "gdsn.class"),
  definition = function(x, do.NULL = TRUE, prefix = NULL){
    read.gdsn(index.gdsn(getfolder.gdsn(x), read.gdsn(index.gdsn(getfolder.gdsn(x), "paths"))[1]))
  }
)

# Standard eset functions to grab matrices, including indexing:
# Calling gdsn node but will allow direct subsetting with '['
# Alternative work around would be to to setGeneric("betas") function(object, ...)
...
setGeneric("QCunmethylated", function(object){standardGeneric("QCunmethylated")})
setMethod(
  f = "QCunmethylated",
  signature(object = "gds.class"),
  definition = function(object){
...
setMethod(
  ...
  definition = function(bn){
    ...
    n.t <- add.gdsn(bn,
                    name = "betas",
                    valdim = c(objdesp.gdsn(index.gdsn(f, "betas"))$dim[1], 0),
                    val = NULL,
                    storage = "float64",
                    replace = TRUE)
...
setMethod(
  ...
  definition = function(mns, fudge = 100, ret2 = FALSE, node = "betas", ...){
    history.submitted <- as.character(Sys.time())
    if(mns$readonly) stop("gds object is in Read-Only mode, please reload!")   
    object <- mns
    ...
    nanes.gds(gds = object,
              ...
              mns = index.gdsn(object, "methylated"),
              uns = index.gdsn(object, "unmethylated"),
              onetwo = fData(object)[, grep('DESIGN', colnames(fData(object)), ignore.case = TRUE)[1]],
              fudge,
              ret2
              )
...
setMethod(
   ...
   signature(object = "gds.class"),
   definition = function(object){
      exp <- data.frame(betas(object)[,, name = TRUE], row.names = rownames(object),
...
setMethod(
  f = "pfilter",
  signature(mn = "gds.class"),
  definition = function(mn, perCount = NULL, pnthresh = NULL, perc = NULL, pthresh = NULL ){
    if(mn$readonly) stop("gds object is in Read-Only mode, please reload!")
    history.submitted <- as.character(Sys.time())
    object <- mn
...
setMethod(
  ...
  definition = function(x, i, j, ..., drop = FALSE)  {
    ...
    if("betas" %in% nodules){
      ...
      n.t <- add.gdsn(f, name = "betas", valdim = c(objdesp.gdsn(betas(x))$dim[1],0),
                      val = NULL, storage = trait)
      for(a in 1:objdesp.gdsn(betas(x))$dim[2]){
        append.gdsn(node = n.t, val = readex.gdsn(betas(x), sel=list(NULL, a)))
      }
      # Replace Old Node with Subset colbycol
      n.n <- add.gdsn(x, name = "betas", valdim = c(length(i), 0), val = NULL,
                      replace = TRUE, storage = trait)
      for(z in j){
        append.gdsn(node = n.n, val = readex.gdsn(betas(f), sel=list(i, z)))
      }
    }

    if("pvals" %in% nodules){
      trait <- tolower(objdesp.gdsn(pvals(x))$trait) 
      n.t <- add.gdsn(f, name = "pvals", valdim = c(objdesp.gdsn(pvals(x))$dim[1],0),
                      val = NULL, storage = trait)
      for(a in 1:objdesp.gdsn(pvals(x))$dim[2]){
        append.gdsn(node = n.t, val = readex.gdsn(pvals(x), sel = list(NULL, a)))
      }
      # Replace Old Node with Subset colbycol
      n.n <- add.gdsn(x, name = "pvals", valdim = c(length(i), 0), val = NULL, replace = TRUE, storage = trait)
      for(z in j){
        append.gdsn(node = n.n, val = readex.gdsn(pvals(f), sel = list(i, z)))
      }  
    }

    if("methylated" %in% nodules){
      trait <- tolower(objdesp.gdsn(methylated(x))$trait)
      n.t <- add.gdsn(f, name="methylated", valdim=c(objdesp.gdsn(methylated(x))$dim[1],0), val = NULL, storage = trait)
      for(a in 1:objdesp.gdsn(methylated(x))$dim[2]){
        append.gdsn(node = n.t, val = readex.gdsn(methylated(x), sel=list(NULL,a)))
      }
      # Replace Old Node with Subset colbycol
      n.n <- add.gdsn(x, name="methylated", valdim=c(length(i), 0), val=NULL, replace=TRUE, storage = trait)
      for(z in j){
        append.gdsn(node = n.n, val = readex.gdsn(methylated(f), sel=list(i, z)))
      } 
    }

    if("unmethylated" %in% nodules){
      trait <- tolower(objdesp.gdsn(unmethylated(x))$trait)
      n.t <- add.gdsn(f, name="unmethylated", valdim=c(objdesp.gdsn(unmethylated(x))$dim[1],0), val = NULL, storage = trait)
      for(a in 1:objdesp.gdsn(unmethylated(x))$dim[2]){
        append.gdsn(node = n.t, val = readex.gdsn(unmethylated(x), sel=list(NULL,a)))
      }
      # Replace Old Node with Subset colbycol
      n.n <- add.gdsn(x, name="unmethylated", valdim=c(length(i), 0), val=NULL, replace=TRUE, storage = trait)
      for(z in j){
        append.gdsn(node = n.n, val = readex.gdsn(unmethylated(f), sel=list(i, z)))
      } 
    }
    ...
    if("NBeads" %in% nodules){
      ...
      n.t <- add.gdsn(f, name="NBeads", valdim=c(objdesp.gdsn(nb)$dim[1],0), val = NULL, storage = trait)
      for(a in 1:objdesp.gdsn(nb)$dim[2]){
        append.gdsn(node = n.t, val = readex.gdsn(nb, sel=list(NULL,a)))
      }
      # Replace Old Node with Subset colbycol
      nb2 <- index.gdsn(f, "NBeads")
      n.n <- add.gdsn(x, name="NBeads", valdim=c(length(i), 0), val=NULL, replace=TRUE, storage = trait)
      for(z in j){
        append.gdsn(node = n.n, val = readex.gdsn(nb2, sel=list(i, z)))
      } 
    }

    # The other objects are small enough to warrant not worrying about memory use.
    if("fData" %in% nodules){  
    ...
    if("QCunmethylated" %in% nodules){  
     ...
     history.command <- paste("Subset of", dim[1], "rows and", dim[2], "samples")
     h <- data.frame(
            submitted = history.submitted,
            finished = history.finished,
            command = history.command,
...
# outlyx - instead of utilizing prcomp.gdsn (since pcout isn't bigmelon friendly - we opt for generic use of prcomp)
setMethod(
  f = "outlyx",
  signature(x = "gdsn.class"),
  definition = function(x, iqr = TRUE, iqrP = 2, pc=1,
                        mv = TRUE, mvP = 0.15, plot = TRUE, perc = 0.01, ...){
...
setMethod(
  ...
  betas <- as.data.frame(na.omit(betas[names(coeff)[-1], ,name = TRUE, drop = FALSE]))
...
setMethod(
  ...
  definition = function(norm, raw){
    ...
    if(!all(dimnorm == dimraw)) stop("Supplied nodes are not the same dimensions")
    res <- list()
...
setMethod(
  ...
  definition = function(object, mul = 4){
    if(object$readonly) stop("gds object is in Read-Only mode, please reload!")
    history.submitted <- as.character(Sys.time())
    bet <- betas(object)
...
setMethod( # Not mem eff # Method not working either!
  f = "seabi",
  signature(bn = "gds.class"),
  definition = function(bn, stop = 1, sex=pData(bn)$sex, X = fData(bn)$CHR == "X" ){
    object    <- bn
