...
# function to check whether a file is already linked to by an existing gds object
# in the current workspace (global env)
findgdsobj <- function(gds){
  fhandle <- gds
  for(obj in ls(envir = .GlobalEnv)){ 
    if(inherits(get(obj), 'gds.class') && get(obj)$filename == normalizePath(gds)){
      temp <- deparse(obj)
      temp <- gsub("[[:blank:]\"]", "", temp)
      warning(paste(get(obj)$filename, "is already open, appending to the linked gds object"))
...
handle <- function(gds){
	...
	#filepath given
    	...
    	#the specified file doesnt exist -> new file to be created
		else if ( !(file_test('-f', gds)) ) {
   			#handle <- newgds(gds) 
   			handle <- gds
   ...
   #gds is not a valid gds.class object or filepath
   	...
   	stop(paste(substitute(gds),"is not a valid gds.class object or filepath"))
   }
   list(handle,newfile)
}
...
app2gds <- function(m, bmln){
  ...
  if(!(newfile)){
    ...
    append.gdsn(index.gdsn(bmln, "unmethylated"), val = unmethylated(m))
    }
    if(exists("NBeads", assayData(m))){
    message('NBeads... ' )
    append.gdsn(index.gdsn(bmln, "NBeads"), val = assayDataElement(m, "NBeads"))
    }
    ...
    message('pData... ' )
    mpdat <- data.frame(lapply(pData(m), as.character), stringsAsFactors = FALSE)
    ...
    message('qcdata... ' )
      if(class(index.gdsn(bmln,'QCmethylated', silent = TRUE)) != 'NULL'){
        append.gdsn(index.gdsn(bmln, "QCmethylated"), val = methylated(QCdata(m)))
      }
      if(class(index.gdsn(bmln, 'QCunmethylated', silent = TRUE)) != 'NULL'){
        append.gdsn(index.gdsn(bmln, "QCunmethylated"), val = unmethylated(QCdata(m)))
      }
    }
    #QC row names should be the same for the appended data, no need to add
    #append.gdsn(index.gdsn(bmln, "QCrownames"), val=rownames(a))
    #add append operation to history 
    numsamp <- nrow(pData(m))
    history.finished <- as.character(Sys.time())
    history.command <- as.character(paste(numsamp,"arrays appended (bigmelon)"))
    h <- data.frame(
    ...
    mhist <- data.frame(lapply(m@history, as.character), stringsAsFactors=FALSE)
...
# add.gdsn(node=bn,name='betas',val=cooked,replace=TRUE)

#  add data from idat files for one barcode to a gds file
#      warn and create if gds doesn't exist
#      warn and do nothing if barcode already in
...
iadd2 <- function(path, gds, chunksize = NULL, ...){
  ...
  if(!is.null(chunksize)){
    ...
    for(i in chunks){
      ...
      ml <- methylumIDATepic(barcodes = sets[!is.na(sets)], n = TRUE, oob = FALSE, idatPath = path)
      gdsfile <- app2gds(ml, gdsfile)
    }
  } else {
    ml <- methylumIDATepic(barcodes, n = TRUE, oob = FALSE, idatPath = path)
    gdsfile <- app2gds(ml, gdsfile)
  }
  gdsfile
}
