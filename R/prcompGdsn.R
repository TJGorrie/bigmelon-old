###
 ...
 # [2] Number of learned principal components is optional, through method dispatch
 #     5 are default, however this is assuming that majority of variance can be
 #     found in the first few PCs hence why the output is somewhat slimmed down.
 #     No Notable time save (May affect timesave if retx=T)
 #     Due to how the principal components are learned it is not a memory efficient operation
 #     Therefore the forthcoming approach is chosen:
 # [3] Perform pca on a subset of data (optionally), by default we opt for 1% 
 #     As it is felt that considering the wide berth of probes 1% is still enough to
 #     learn the rough indication of the principal components. 
 #     Notable Time save (obviously), Preserves memory usage as svd is performed on a 
 #     small subset.
 #
### 
prcomp.gdsn.class <- function(x, center = FALSE, scale. = FALSE, tol= NULL, rank.=NULL, retx=FALSE, perc=0.01, npcs = NULL, ...){
  # x = node 
  dim <- objdesp.gdsn(x)$dim
  f <- createfn.gds("temp.gds", allow.duplicate=TRUE)
  n.t <- add.gdsn(f, val = NULL, valdim = c(dim[1], 0), replace = TRUE, name = "scale", storage = "float64")   
  x0 <- x
  cen <- NULL
  sc <- NULL
  if(center|scale.){ # If normalized Center | Scale = FALSE
  # Would apply.gdsn suffice here? Is scale necessary IF data is also normalized.
  for(i in 1:dim[2]){
    val <- readex.gdsn(x, sel = list(NULL,i))
    y <- scale(val, center = center, scale = scale.)
    if(!is.null(attr(y, "scaled:center"))) cen[i] <- attr(y, "scaled:center")
    if(!is.null(attr(y, "scaled:scale"))) sc[i] <- attr(y, "scaled:scale")
    append.gdsn(n.t, y)
  }
  x <- n.t 
  }
...
#    dimnames(s$v) <- list(colnames(x), paste0("PC", j)) # Broken, fix later
    r <- list(sdev = s$d, rotation = s$v,
              center = if(is.null(cen)) FALSE else cen,
              scale = if(is.null(sc)) FALSE else sc)

  # There isn't a clever way to perform matrix multiplications.
  if (retx) {
