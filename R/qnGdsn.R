###
 # "Memory Efficient Quantile Normalizer"
 # Contains a somewhat stripped down version of limma::normalizeQuantiles
 # With Ties method parameter hard-coded.
 # Configured to interact with gdsfmt objects.
 #
###
 # Derivatives:
 # design.qn.gdsn | location: designqnGdsn.R
 # Which is an alternative to this, to handle differences in probe-design simultaneously.
 #
###
 # Arguments:
 # gds      : GDS object where data is to be placed. of gds.class
 # target   : Node to be Normalized, of gdsn.class
 # newnode  : "String" of the name of the node to initialized. length(newnode)==1
 #
###
...
qn.gdsn <- function(gds, 
  ...
  for (x in 1:dim[2]){
    ...
    if(nobs[x] < dim[1]) {
      isna <- is.na(val)
      val[!isna] <- approx(i, rm, (r[!isna]-1)/(nobs[x]-1), ties = "ordered")$y
    } else {
